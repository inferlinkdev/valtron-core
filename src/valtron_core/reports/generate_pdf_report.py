...
_STYLES: dict[str, ParagraphStyle] = {
    "h1": ParagraphStyle(
        "rl_h1", fontName="Helvetica-Bold", fontSize=18, textColor=C_BLUE, leading=22
    ),
    "h2": ParagraphStyle(
        "rl_h2", fontName="Helvetica-Bold", fontSize=13, textColor=C_BLUE, leading=17, spaceAfter=4
    ),
    "h3": ParagraphStyle(
        "rl_h3",
    ...
    "body": ParagraphStyle(
        "rl_bod", fontName="Helvetica", fontSize=10, textColor=C_DARK, leading=14, spaceAfter=6
    ),
    "small": ParagraphStyle(
        "rl_sm", fontName="Helvetica", fontSize=8.5, textColor=C_MEDIUM, leading=12
    ),
    "timestamp": ParagraphStyle(
        "rl_ts", fontName="Helvetica", fontSize=8, textColor=C_LIGHT, leading=11
    ),
    "note": ParagraphStyle(
        "rl_nt", fontName="Helvetica-Oblique", fontSize=8, textColor=C_MEDIUM, leading=11
    ),
    ...
    "field_note": ParagraphStyle(
        "rl_fn", fontName="Helvetica-Oblique", fontSize=8.5, textColor=C_SUBHEAD, leading=11
    ),
    "app_body": ParagraphStyle(
        "rl_ab", fontName="Helvetica", fontSize=9.5, textColor=C_APP_BODY, leading=13
    ),
    "app_subhead": ParagraphStyle(
        "rl_as",
        fontName="Helvetica-Bold",
        fontSize=10.5,
        textColor=C_BLUE,
    ...
    "td_cell": ParagraphStyle(
        "rl_tdc", fontName="Helvetica", fontSize=9, textColor=C_DARK, leading=11
    ),
}
...
class PdfReportGenerator(_ReportBase):
    ...
    def generate_pdf_report(
        ...
        Args:
            ...
            output_path: Directory the PDF is written into.
            recommendation: Pre-generated recommendation text to include; unlike the HTML
                report generator, this method does not generate one itself.
            original_prompt: The unmodified prompt template shown in the report.
        ...
        has_optimizations = prompt_optimizations is not None and any(
            len(v) > 0 for v in prompt_optimizations.values()
        )
    ...
    def _format_metric_display(metric: str, params: dict[str, Any]) -> str:
        ...
        name = (
            "Aggregated (weighted sub-fields)"
    ...
    def _build_summary(self, data: dict) -> list[Any]:
        ...
        elems.append(Paragraph(f"<b>Models Evaluated:</b> {data['num_models']}", _STYLES["body"]))
        elems.append(
            Paragraph(f"<b>Documents Tested:</b> {data['num_documents']}", _STYLES["body"])
        )
    ...
    def _build_perf_table(self, data: dict) -> list[Any]:
        ...
        def _td(text: str, is_best: bool = False) -> Paragraph:
            ...
            if is_best:
                return Paragraph(f'<b><font color="#2563eb">{safe}</font></b>', _STYLES["td_cell"])
        ...
        for row_idx, result in enumerate(results):
            ...
            best_avg_cost = (
                result.metrics.average_cost_per_document == performance_best["best_avg_cost"]
            )
            best_total_time = result.metrics.total_time == performance_best["best_total_time"]
            best_avg_time = (
                result.metrics.average_time_per_document == performance_best["best_avg_time"]
            )
    ...
    def _build_field_metrics(self, data: dict) -> list[Any]:
        ...
        for field_name in data["all_field_names"]:
            ...
            block: list[Any] = [
                ...
                Paragraph(f"Scored using: {_xml.escape(' / '.join(seen))}", _STYLES["field_note"]),
            ]
    ...
    def _build_recommendation(self, data: dict) -> list[Any]:
        ...
        elems.append(Paragraph(_xml.escape(rec).replace("\n", "<br/>"), _STYLES["rec"]))
        return elems
    ...
    def _build_appendix_b(self, data: dict) -> list[Any]:
        ...
        def _entry(title: str, tech_term: str | None, body: str) -> list[Any]:
            ...
            if tech_term:
                heading += f' <font color="#6b7280" size="8">({_xml.escape(tech_term)})</font>'
            return [
        ...
        elems.extend(
            _entry(
                "When the field holds a single value",
                None,
        ...
        elems.extend(
            _entry(
                "When the field holds a list of items",
                None,
        ...
        elems.extend(
            _entry(
                "When the field holds an object or dictionary",
                None,
        ...
        elems.extend(
            _entry(
                "When a value field is nested inside a list object",
                None,
    ...
    def _build_appendix_a(self, data: dict) -> list[Any]:
        ...
        for result in data["results"]:
            ...
            if override:
                mn = result.model.split("/")[-1] if "/" in result.model else result.model
                elems.append(
                    Paragraph(f"<b>Override Prompt - {_xml.escape(mn)}</b>", _STYLES["h4"])
                )
