...
- [`ExtractionExperiment`](../api/extraction_experiment) with `response_format` set to a Pydantic schema
- Multi-institution grading across several affiliations
- [`field_metrics_config`](../api/field_metrics_config) for unordered list scoring with fuzzy name matching
...
- Pass `response_format=AffiliationResult` to `ExtractionExperiment`; it requires a schema and fails immediately without one.
...
- Try the `decompose` manipulation to call the model once per field. See [Prompt Manipulations](../user-guide/manipulations/index).
- See [Field Metrics](../user-guide/extraction/field-metrics/index) for all available comparison strategies.
