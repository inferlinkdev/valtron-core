...
- `ClassificationExperiment.load_experiment_results()` to restore a prior run
...
```python
experiment = ClassificationExperiment.load_experiment_results("examples/results/incremental")
...
- Use `save_pdf_report()` after loading to produce a PDF without re-evaluating. See [Evaluation API: Running an Evaluation](../user-guide/classification/evaluation-api.md#running-an-evaluation).
