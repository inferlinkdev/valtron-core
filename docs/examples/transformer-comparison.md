...
- Training a local [`TransformerClassifier`](../api/transformer_classifier) from labeled data
...
- Transformer models only work with `ClassificationExperiment` (no `response_format`).
...
- Run the trained model standalone with `TransformerModelWrapper`. See [Transformer Models: Direct Inference](../user-guide/self-hosting/transformer-models.md#direct-inference).
